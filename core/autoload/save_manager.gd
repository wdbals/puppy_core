extends Node
## Singleton utilitario para escritura y lectura de módulos de guardado.
...
# Helper privado para evitar repetir código en los write
func _write_module_data_to_config(config: ConfigFile, module: SaveModule) -> void:
	...
	# Actualizamos timestamp global cada vez que escribimos algo
...
## Lee múltiples módulos en una sola operación de disco
...
func _load_config_file(slot_name: String) -> ConfigFile:
	...
	if EngineConfig.SAVE_USE_ENCRYPTION:
		if FileAccess.file_exists(path): # Chequeo extra de seguridad
			err = config.load_encrypted_pass(path, _SECRET_KEY)
	else:
		err = config.load(path)

	# Si falla la carga, devolvemos un config vacío pero válido para no crashear
	if err != OK:
		push_warning("No se pudo cargar config: " + path)
		pass
